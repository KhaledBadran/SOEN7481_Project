...
def f1(y_true, y_pred):
    return (
        2
        * tp(y_true, y_pred)
        / (2 * tp(y_true, y_pred) + fp(y_true, y_pred) + fn(y_true, y_pred))
    )


# AUC
def auc(y_true, y_pred):
    return (tpr(y_true, y_pred) + tnr(y_true, y_pred)) / 2


# MCC
def mcc(y_true, y_pred):
    return (
        tp(y_true, y_pred) * tn(y_true, y_pred)
        - fp(y_true, y_pred) * fn(y_true, y_pred)
    ) / np.sqrt(
        (tp(y_true, y_pred) + fp(y_true, y_pred))
        * (tp(y_true, y_pred) + fn(y_true, y_pred))
