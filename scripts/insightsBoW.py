...
def fitModelAndCheckWords(tokenizer, tokenizerFlaky, tokenizerNonFlaky, body, data):
    ...
    for word in importantWords:
        print(word)
        print(
            "Number of occurence in Flaky Tests", tokenizerFlaky.word_counts.get(word)
        )
        print(
            "Number of occurence in Non Flaky Tests",
            tokenizerNonFlaky.word_counts.get(word),
        )


def featuresUnderstanding(tokenizer, classifier, num):
    featureImportances = classifier.feature_importances_
    featureImportancesSorted = sorted(
        range(len(featureImportances)),
        key=lambda k: featureImportances[k],
        reverse=True,
    )
